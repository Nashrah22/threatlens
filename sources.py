...
def get_virustotal(value: str, ioc_type: str, _api_keys: dict) -> dict:
    ...
    try:
        ...
        # Score is based on how many engines flagged it, not diluted by the
        # total engine count — with 90+ engines scanning, even 5 "malicious"
        # hits is a strong signal and should score accordingly, not ~5/100.
        if malicious >= 10:
            risk_score = min(100, 90 + (malicious - 10))
        elif malicious >= 5:
            risk_score = 70 + (malicious - 5) * 4
        elif malicious >= 1:
            risk_score = 40 + malicious * 6
        elif suspicious >= 1:
            risk_score = 15 + min(15, suspicious * 3)
        else:
            risk_score = 0
